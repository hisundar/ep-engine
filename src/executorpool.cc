...
size_t ExecutorPool::_getMaxNonIO(void) {
    // if configured value is 0 (not specified) use the derived value as max
    return maxNumNonIO ? maxNumNonIO : getNumNonIO();
}
...
size_t ExecutorPool::_getMaxAuxIO(void) {
    // if configured value is 0 (not specified) use the derived value as max
    return maxNumAuxIO ? maxNumAuxIO : getNumAuxIO();
}

size_t ExecutorPool::_getMaxWriters(void) {
    // if configured value is 0 (not specified) use default value
    return maxNumWriters ? maxNumWriters : EP_MAX_WRITERS_DEFAULT;
}
...
size_t ExecutorPool::getNumWriters(void) {
    ...
    return std::min(count, _getMaxWriters());
}

size_t ExecutorPool::_getMaxReaders(void) {
    // if configured value is 0 (not specified) use default value
    return maxNumReaders ? maxNumReaders : EP_MAX_READERS_DEFAULT;
}
...
size_t ExecutorPool::getNumReaders(void) {
    ...
    return std::min(count, _getMaxReaders());
}
...
ExecutorPool::ExecutorPool(size_t maxThreads, size_t nTaskSets,
                  ...
                  numTaskSets(nTaskSets), maxNumReaders(maxReaders),
                  maxNumWriters(maxWriters), maxNumAuxIO(maxAuxIO),
                  maxNumNonIO(maxNonIO), numReadyTasks(0),
...
bool ExecutorPool::_startWorkers(void) {
    ...
    maxWorkers[READER_TASK_IDX] = std::min(threadQ.size(), _getMaxReaders());
    maxWorkers[WRITER_TASK_IDX] = std::min(threadQ.size(), _getMaxWriters());
    maxWorkers[AUXIO_TASK_IDX]  = std::min(threadQ.size(), _getMaxAuxIO());
    maxWorkers[NONIO_TASK_IDX]  = std::min(threadQ.size(), _getMaxNonIO());
