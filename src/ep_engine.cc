...
ENGINE_ERROR_CODE EventuallyPersistentEngine::doSchedulerStats(const void
                                                                *cookie,
                                                                ADD_STAT
                                                                add_stat) {
    for (type_id_t i = (type_id_t)0; i < MAX_TYPE_ID; ++i) {
        add_casted_stat(Priority::getTypeName(i), stats.schedulingHisto[i],
                        add_stat, cookie);
    }
...
ENGINE_ERROR_CODE EventuallyPersistentEngine::doRunTimeStats(const void
                                                                *cookie,
                                                                ...
                                                                add_stat) {
    for (type_id_t i = (type_id_t)0; i < MAX_TYPE_ID; ++i) {
        add_casted_stat(Priority::getTypeName(i), stats.taskRuntimeHisto[i],
                        add_stat, cookie);
    }
